...
7. Read `docs/context/hooks/README.md` and confirm the current timing, matched hooks, and hard gates
8. Read more project context only when needed
9. If resuming a version/change, read the relevant `session-memory.md`
10. Before each stage starts a concrete task, re-index `docs/context/experience/index.md` and load any mature experience that can be reused or fused into the work

Do not bulk-load the whole `docs/context/` tree.
...
16. After each stage-level task completes, review what was learned, capture it in `session-memory.md`, and promote or fuse mature experience into `docs/context/experience/` or formal rules when justified
...
- Durable repository knowledge belongs in `docs/context/` and `docs/memory/`
