...
- sug-01: 修复 regression 命令：--confirm 会消费 regression 导致 --testing 无法使用
- sug-02: rename 目录丢失 id 前缀 + 未同步 state yaml（req-25 final regression 发现 P1-new-01）
- sug-03: harness next 不更新 state yaml 的 stage/status，导致需手动改 yaml 才能 archive（req-25 P1-new-02）
- sug-04: regression 产出目录含空格、无 id 前缀（req-25 P1-new-03）
- sug-05: archive 路径出现 archive/main/main/ 双层 branch（req-25 P1-new-04）
- sug-06: # 制品定义收敛：只输出用户需要关注的出品
