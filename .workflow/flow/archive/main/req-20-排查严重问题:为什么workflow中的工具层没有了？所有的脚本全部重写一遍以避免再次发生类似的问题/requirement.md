...
- **chg-01**：修复 `LEGACY_CLEANUP_TARGETS` 和 `_required_dirs`，移除对 `.workflow/tools/` 的误清理并保留目录创建
- **chg-02**：重建 `assets/scaffold_v2/.workflow/tools/` 模板（从 backup/legacy-cleanup 恢复并整理）
- **chg-03**：恢复当前仓库的 `.workflow/tools/` 目录
- **chg-04**：补充/更新测试用例，验证 tools 层保护和同步正确性
