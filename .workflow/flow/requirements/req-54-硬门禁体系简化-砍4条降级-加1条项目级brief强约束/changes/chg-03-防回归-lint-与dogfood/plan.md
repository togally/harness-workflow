...
### F1：新增 `tests/test_req54_hard_gate_simplify.py`（约 200~300 行）

骨架：

```python
"""req-54 / chg-03: 硬门禁体系简化防回归 lint + dogfood。
...
文件落位：`tests/test_req54_hard_gate_simplify.py`（仓库根 tests/ 子树，约 220 行）。

## 2. 实施步骤

1. **Step 1**：创建 `tests/test_req54_hard_gate_simplify.py`（≥ 6 TC，按上方骨架）。
2. **Step 2**：跑 `pytest tests/test_req54_hard_gate_simplify.py -v` → ≥ 6 PASS。
3. **Step 3**：跑全 suite `pytest tests/ -x --tb=short`（baseline 不增 fail 数）。
4. **Step 4**：跑 `harness validate --contract all` → exit 0。

## 4. 测试用例设计

> regression_scope: full（含 dogfood TC-Dogfood-06，整个 install + validate 链路被覆盖；任何 contract 因 chg-01 / chg-02 改动 break 都会被 TC-Dogfood-06 抓住）
> 波及接口清单（git diff --name-only 自动生成 + 人工补全）：
> - tests/test_req54_hard_gate_simplify.py（新增）
> - 间接波及（被 TC 覆盖的）：WORKFLOW.md / base-role.md / harness-manager.md / stage-role.md（live + mirror 各 1）+ harness CLI install / validate 子命令

| 用例名 | 输入 | 期望 | 对应 AC | 优先级 |
|-------|------|------|---------|--------|
| TC-01 | grep 计 WORKFLOW.md `^## 全局硬门禁` 段编号行数 | ≤ 2 | AC-01 | P0 |
| TC-02 | grep base-role.md（live + mirror）「## 工具委派指导原则」+「## 操作日志指导原则」标题 | 各 1 命中 / 各 1 命中 | AC-02 / AC-03 | P0 |
| TC-03 | grep base-role.md（live + mirror）「## 硬门禁八」段 + 段内 Step 7.6 / 7.6.1 / artifacts/project/ / scope 枚举 | 段命中 + 字面命中 | AC-04 | P0 |
| TC-04 | grep harness-manager.md（live + mirror）§3.6.2 段 + 段内 boilerplate | 段命中 + 字面命中 | AC-05 | P0 |
| TC-05 | subprocess `diff -q` 4 对 live ↔ mirror | returncode == 0（4 对全 silent） | AC-07 | P0 |
| TC-Dogfood-06 | tmp_path → git init → `harness install --force-managed` → `harness validate --contract all` | exit 0 + exit 0 | AC-10 | P0 |

**TC-Dogfood-06 必填字段（sug-52）**：
- 用例名：TC-Dogfood-06
- tmpdir fixture：`tmp_path` (pytest builtin)
- 子进程命令：`subprocess.run([sys.executable, '-m', 'harness_workflow.cli', 'install', '--force-managed'])` + `subprocess.run([sys.executable, '-m', 'harness_workflow.cli', 'validate', '--contract', 'all'])`
- stdout 断言：returncode == 0（两步均 exit 0）
- runtime stage 断言：N/A（fresh repo install 后 stage 默认未定义；本 TC 仅校验 install + validate 链路 PASS 即可）
...
```bash
# AC-09：6 TC 全 PASS
pytest tests/test_req54_hard_gate_simplify.py -v   # 期望 ≥ 6 PASS

# AC-10：fresh repo dogfood
# （由 TC-Dogfood-06 内嵌 subprocess 调用覆盖，亦可手动跑）
cd /tmp/test-fresh && git init && python3 -m harness_workflow.cli install --force-managed && python3 -m harness_workflow.cli validate --contract all
# 期望：两步 exit 0

# 全 suite 不增 fail 数
pytest tests/ -q --tb=no   # 期望 fail 数 ≤ baseline（51 failed by req-53 done report）
```
...
|-----------|-----------|---------|
| `tests/test_req54_hard_gate_simplify.py` | （tests/ 不在 scaffold_v2 镜像范围；硬门禁五保护面 §1-§5 不含 tests/，无需 mirror） | N/A |

实施约束：tests/ 子树不在 scaffold_v2 mirror 范围（参见 harness-manager.md §硬门禁五例外白名单）；无 mirror 同步要求。
