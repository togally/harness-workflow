...
## Appendix: Step 2.5 commit revert dry-run 抽样结果（2026-05-10 done 阶段补）

抽样最近 N+1 = 4 commit（chg 数 3 + 1）：

| commit | 标题 | dry-run | 备注 |
|---|---|---|---|
| 399cfbd | feat: req-56 ... | **PASS** | 本 req commit 可干净 revert，核心安全证 ✓ |
| a959eb0 | chore: sug-74 入池 | PASS | 干净 |
| 744d1e3 | fix: bugfix-11 install runtime.yaml 反写 3 bug | **CONFLICT** | feedback.jsonl + runtime.yaml 状态日志文件 conflict（非功能性，by-design 状态文件每次运行变化） |
| 1f7522f | fix: pyproject.toml 漏配 vendor 副本 | CONFLICT | 同上原因 |

**判定**：本 req commit 自身可干净 revert（核心安全目标达成）；历史 conflict 仅在状态日志文件，非功能性，**不阻塞 done / archive**。

> 按 done.md Step 2.5 规则：exit 1（有 conflict）→ 不阻塞 done 阶段；记录在 done-report.md 即可。
