...
## 经验十三：融合 bugfix（多事项块）的同 commit / 同 mirror 同步实操（bugfix-6 引入）

### 场景

执行融合 bugfix（单一 bugfix 周期内多事项块独立诊断 + 修复点合并）的 executing 阶段，如 bugfix-6（工作流契约统一加固（对人机器分离 + 测试契约重塑）） 的 14 修复点（事项 A: 5 + 事项 B: 6 + 事项 C: 3，B6 降级为 sug 后实际 13 落地），跨 helper / 跨契约文档 / 跨模板 mirror / 跨 lint。

### 经验内容

**commit 颗粒选择**：

1. **强烈建议一事项块一 commit**（如 A 块 / B 块 / C 块各一 commit），不一定按修复点拆 14 commit——后者会让 git log 噪声爆炸 + revert 颗粒过细难以逻辑回滚；
2. **不要"一 bugfix 一巨 commit"**：如果走单巨 commit，事项 A / B / C 任一出问题需要单独回滚时，必须手工 revert 部分 hunk，比一事项块一 commit 麻烦得多；
3. **commit message 写完整事项块描述**：如 `executing(bugfix-6 事项 A): 路径关注点分离强化（A1 ~ A5 / 5 修复点 / artifact-placement lint）` 而非裸 `fix bugfix-6` —— 配合契约 7 + 硬门禁六。

**mirror 同步操作要点**：

1. **scaffold_v2 mirror 同 commit 跟随**：每改一份 `.workflow/context/roles/{role}.md` / `.workflow/evaluation/{file}.md` / `.workflow/flow/repository-layout.md`，必须**同 commit** 同步 `src/harness_workflow/assets/scaffold_v2/.workflow/...` 的对应路径；不要"先改 live 后补 mirror"——容易遗漏 + reviewer 抽样 diff 出非空。
2. **diff -rq 自检**：每事项块 commit 前跑 `diff -rq .workflow/context/roles/ src/harness_workflow/assets/scaffold_v2/.workflow/context/roles/` + `diff -rq .workflow/evaluation/ scaffold_v2/.workflow/evaluation/` + `diff -q .workflow/flow/repository-layout.md scaffold_v2/.workflow/flow/repository-layout.md`，断言空（除运行时目录 / 白名单豁免文件）。
3. **模板 mirror 双语同步**：如 `change-plan.md.tmpl` + `.en.tmpl` 双语模板，同 commit 一并改；其他 mirror 路径（`.claude/skills/...` / `.codex/...` / `.qoder/...` / `build/lib/...`）按 sug-15 经验"模板差异对比"扫一遍。

**lint 双向覆盖契约**：

1. **新增 lint 时同时挂"上下游"**：如 B5 `test-case-design-completeness` lint 同时覆盖 plan.md（req 流程）+ bugfix diagnosis.md（bugfix 流程）—— 用单一 lint 双向覆盖，比拆两个 lint 维护成本低。
2. **lint 自证**：lint 落地后**必须**在自身 bugfix 周期跑一次（如 bugfix-6 自身的 diagnosis.md §测试用例设计 18 条用例就是 B5 lint 的 dogfood 自证）；自证失败立即修，避免"lint 落地但生产环境跑不通"。

**降级为 sug 的边界**：

- 修复点超 ≤ 12 上限时，优先把"非主路径 / WARN 类 / 话术拦截类"修复点降级为 sug（如 bugfix-6 B6 降级为 sug-33（briefing 话术 lint）），不要硬塞进 scope；
- 降级 sug 必须在 bugfix.md §修复方案 中明示理由 + default-pick 决策留痕，不要静默丢弃。

### 反例

- 14 修复点拆 14 commit → git log 噪声爆炸，reviewer 抽样难，revert 颗粒过细；正解：3 事项块 3 commit。
- mirror 后补 commit → diff -rq 抽样在 acceptance 阶段才暴露差异，需重新切回 executing 修复，浪费一轮 stage 流转。
- 双语模板只改一边（中文 tmpl 改了，英文 en.tmpl 没改）→ 多语言用户 install 出来的 plan.md 模板缺 §4 章节，contract test 跑通但实际生产环境不一致。
- B5 lint 不在自身 bugfix 周期 dogfood 自证 → bugfix-7+ 真跑时才发现 lint 实现错位（如波及接口判定算法漏 case），属于"lint 落地但 untested"反模式。

### 来源

bugfix-6（工作流契约统一加固（对人机器分离 + 测试契约重塑））— executing 阶段 13 修复点同步实施 + B5 lint dogfood 自证 + scaffold_v2 mirror diff 全量断言
