...
## 经验：testing 产物落点 + heading 精确匹配（req-56 done 复盘）

### 教训

testing 子 agent 落 `test-report.md` 时必须：

1. **路径**：落到 `.workflow/flow/requirements/{req-id}-{slug}/test-report.md` **根目录**，而**不是** `testing/` 子目录。`_is_stage_work_done` work-done gate 只扫根目录路径；落到 `testing/` 子目录会导致 `harness next` 报"Stage testing 工作未完成"被拦截。
2. **heading**：必须含 `## 结论` 或 `## §结论` 二级标题（正则 `(?:^|\n)##\s*§?结论` 匹配）；`### 判定` 三级标题不算。子 agent briefing 必须明确这点。

### 反例

- testing 子 agent 把 test-report.md 写到 `testing/` 子目录 → harness next 拦截
- 用 `### 判定` 表达 PASS/FAIL → harness next 拦截
- 主 agent 只能事后修：mv 文件 + 改 heading 才能继续推进；浪费一次往返
