# Experience Index

> Auto-generated index of `.workflow/context/experience`.

## regression
- [regression/core-py.md](regression/core-py.md) — Regression Experience: core-py
- [regression/harness-update-current-requirement.md](regression/harness-update-current-requirement.md) — Regression Experience: harness-update-current-requirement

## risk
- [risk/known-risks.md](risk/known-risks.md) — Known Risks and Mitigations

## roles
- [roles/acceptance.md](roles/acceptance.md) — Acceptance Stage Experience
- [roles/executing.md](roles/executing.md) — Development Stage Experience
- [roles/planning.md](roles/planning.md) — Planning Stage Experience
- [roles/regression.md](roles/regression.md) — Regression Stage Experience
- [roles/requirement-review.md](roles/requirement-review.md) — Requirement Stage Experience
- [roles/testing.md](roles/testing.md) — Testing Stage Experience

## tool
- [tool/claude-code-context.md](tool/claude-code-context.md) — Claude Code 上下文管理工具经验
- [tool/harness-ff.md](tool/harness-ff.md) — Harness Fast-Forward (ff) Experience
- [tool/harness.md](tool/harness.md) — Harness Tool Experience

---

Update this index by running `harness update`.
