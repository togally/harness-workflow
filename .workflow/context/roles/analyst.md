...
|---|---|---|
| `# Design: <title>` 头部（title / Generated by / Branch / date） | frontmatter | 重写 yaml `id` / `title` / `created_at` / `operation_type` / `stage="analysis"` |
| `## Problem Statement` + `## Demand Evidence` + `## Target User & Narrowest Wedge` | `## Goal` | 汇总成 2~3 条 bullet：核心问题 + 用户价值 + 验证证据 |
| `## Constraints` + `## Recommended Approach` | `## Scope.Included` | 提炼为可交付项清单 |
| `## Approaches Considered`（"未选"分支） | `## Scope.Excluded` | 列"已比选但不采用"方向 |
| `## Success Criteria` | `## Acceptance Criteria` | 强对齐：逐条编号化为 AC-01 / AC-02 ...；每条格式 `- AC-NN: [简短描述]` |
| `## Next Steps` / `## The Assignment` | `## Split Rules` | 转为 chg 拆分原则 + 渐进扩展规划 |
| 多余段（`## Premises` / `## Cross-Model Perspective` / `## Open Questions` / `## Distribution Plan` / `## Dependencies` / `## What I noticed about how you think` / `## Reviewer Concerns`） | `## Office Hours Notes` | 整体追加到 requirement.md 末尾作附加段，保留 Spec Review Loop 思考价值 |

**builder mode 段映射**（段名差异处理）：

| design doc 段（builder mode） | requirement.md 段 | 处理方式 |
|---|---|---|
| `## Problem Statement` + `## What Makes This Cool` | `## Goal` | 同 startup：汇总 2 条 bullet |
| `## Constraints` + `## Recommended Approach` | `## Scope.Included` | 同 startup |
| `## Approaches Considered`（未选分支） | `## Scope.Excluded` | 同 startup |
| `## Success Criteria` | `## Acceptance Criteria` | 同 startup：编号化 AC-NN |
| `## Next Steps` | `## Split Rules` | 同 startup |
| 多余段（同 startup 多余段） | `## Office Hours Notes` | 同 startup |
