...
For Qoder, the most reliable repository entrypoints are `/harness`, `/harness-requirement`, `/harness-change`, `/harness-plan`, and the other generated `harness-*` commands under `.qoder/commands/`.
For Claude Code, the same command family is generated under `.claude/commands/`.
For Codex, the same command family is exposed as thin `.codex/skills/harness-*` wrappers around the main Harness skill.
