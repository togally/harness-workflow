...
def recommend_actions(root: Path, facts: dict[str, object]) -> list[str]:
    ...
    if not facts["has_constitution"]:
        actions.append("Add docs/memory/constitution.md as durable memory root.")
    if not facts["has_experience_index"]:
        actions.append("Add docs/context/experience/index.md for keyword-based experience loading.")
    if not facts["has_agent_workflow"]:
        actions.append("Add docs/context/rules/agent-workflow.md for detailed workflow rules.")
    return actions
