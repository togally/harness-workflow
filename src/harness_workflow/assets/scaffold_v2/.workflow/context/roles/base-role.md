...
**批量列举子条款（reg-01（对人汇报批量列举 id 缺 title 不可读） / chg-06（硬门禁六 + 契约 7 批量列举子条款补丁））**：当同一句 / 同一行 / 同一表格单元内并列 ≥ 2 个**不同** id（DAG 完成度、收束汇报、跨 chg 索引、进度概览等场景），每一条 id 都必须紧跟 ≤ 15 字描述，**禁止**合并为 `chg-01 / 02 / 03 / 04 / 05` / `req-34、35、36` / `chg-{01..05}` 等裸数字扫射形态；本子条款**优先于**上方"同一 id 连续多次引用"例外条款，例外仅适用于"同一 id 重复"，**不适用于**"批量列举多个不同 id"。
- 正例：`本 stage 完成 chg-01（protocols 目录扩展）+ chg-02（触发门禁 lint）+ chg-03（runtime pending 字段）。`
- 反例：`本 stage 完成 chg-01 / 02 / 03。`（裸数字扫射，违反本子条款。）
