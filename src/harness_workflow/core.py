    ...
    meta.update(
        {
            ...
            "approval_required": bool(meta.get("approval_required", False)),
            "stage_entered_at": meta.get("stage_entered_at", "") or datetime.now(timezone.utc).isoformat(),
        }
    )
...
def create_regression(root: Path, name: str | None, regression_id: str | None = None, title: str | None = None) -> int:
    ...
    persist_workflow_state(root, version_id, meta, runtime)
    record_feedback_event(root, "regression_created", {"regression_id": resolved_id, "version": version_id})
...
def workflow_fast_forward(root: Path) -> int:
    ...
    current_version = require_active_version_id(root, config, runtime)
    prev_meta = load_version_meta(root, current_version)
    from_stage = str(prev_meta.get("stage", "idle"))
    meta = apply_stage_transition(prev_meta, fast_forward=True)
