"""Unit tests for `archive_requirement` 路径拼接（req-26 / chg-04, req-29 / chg-01）。
...
覆盖 AC-03（req-29 / chg-01）：
- `resolve_archive_root` 默认优先返回 primary，即使 legacy 非空；
- 仅在显式 opt-in（``prefer_legacy=True`` 或 ``HARNESS_ARCHIVE_LEGACY=1``）
  且 legacy 非空时才返回 legacy。
...
class ResolveArchiveRootTest(unittest.TestCase):
    """req-29 / chg-01（AC-03）：`resolve_archive_root` primary 优先，legacy 仅 opt-in。"""

    def setUp(self) -> None:
        self.tempdir = Path(tempfile.mkdtemp(prefix="harness-resolve-archive-"))
        ...
        self._branch_patch.start()
        # 保存并清理 HARNESS_ARCHIVE_LEGACY 环境变量，防止外部污染
    ...
    def _seed_legacy(self) -> Path:
        """在 legacy 路径下制造一些实质性内容，模拟老仓状态。"""
        legacy = self.root / ".workflow" / "flow" / "archive"
        legacy.mkdir(parents=True, exist_ok=True)
        (legacy / "req-88-old").mkdir(parents=True, exist_ok=True)
        (legacy / "req-88-old" / "requirement.md").write_text(
            "# legacy seed\n", encoding="utf-8"
        )
        return legacy

    def test_resolve_archive_root_returns_primary_by_default(self) -> None:
        """AC-03 核心：legacy 非空 + primary 空，默认仍返回 primary（不降级）。

        额外断言 stderr 打印一次迁移提示（notice 级），而非老版本的 warning 降级。
        """
        from harness_workflow.workflow_helpers import resolve_archive_root
        ...
        self.assertEqual(
            got,
            primary_expected,
            f"默认应返回 primary {primary_expected}，实际返回 {got}",
        )
        stderr_text = stderr_buf.getvalue()
        self.assertIn(
            "harness migrate archive",
            stderr_text,
            f"legacy 非空时 stderr 应提示迁移命令；实际：{stderr_text!r}",
        )
        # 旧版 warning 文本不得再出现（不降级 → 不应再声明 "using legacy"）
        self.assertNotIn(
            "using legacy archive path",
            stderr_text,
            f"默认路径不得再声明 using legacy；实际：{stderr_text!r}",
        )

    def test_resolve_archive_root_returns_legacy_when_opted_in(self) -> None:
        """AC-03 对照：显式 opt-in（参数 或 环境变量）+ legacy 非空 → 返回 legacy。"""
        from harness_workflow.workflow_helpers import resolve_archive_root

        legacy_expected = self._seed_legacy()

        # Case A：参数 opt-in
        got_param = resolve_archive_root(self.root, prefer_legacy=True)
        self.assertEqual(
            got_param,
            legacy_expected,
            f"prefer_legacy=True 应返回 legacy {legacy_expected}，实际 {got_param}",
        )

        # Case B：环境变量 opt-in
        os.environ["HARNESS_ARCHIVE_LEGACY"] = "1"
        try:
            got_env = resolve_archive_root(self.root)
        finally:
            os.environ.pop("HARNESS_ARCHIVE_LEGACY", None)
        self.assertEqual(
            got_env,
            legacy_expected,
            f"HARNESS_ARCHIVE_LEGACY=1 应返回 legacy {legacy_expected}，实际 {got_env}",
        )

        # Case C：opt-in 但 legacy 空 → 仍落 primary（opt-in 不凭空造 legacy 数据）
        empty_root = self.tempdir / "repo-empty"
        empty_root.mkdir(parents=True)
        primary_empty = empty_root / "artifacts" / "main" / "archive"
        got_empty = resolve_archive_root(empty_root, prefer_legacy=True)
        self.assertEqual(
            got_empty,
            primary_empty,
            "opt-in + legacy 空 → 应回落 primary",
        )


class ArchiveIntegrationTest(unittest.TestCase):
    """req-29 / chg-01（AC-03）集成：legacy 非空时新归档仍落 primary。"""

    def setUp(self) -> None:
        self.tempdir = Path(tempfile.mkdtemp(prefix="harness-archive-integration-"))
        self.root = _make_harness_workspace(self.tempdir)
        self._branch_patch = mock.patch(
            "harness_workflow.workflow_helpers._get_git_branch",
            return_value="main",
    ...
    def test_archive_new_requirement_lands_in_primary(self) -> None:
        """端到端：legacy 非空 + 不 opt-in → 新 req archive 结果落到 primary 子树。"""
        from harness_workflow.workflow_helpers import archive_requirement

        # 预置 legacy 历史数据（模拟老仓）
        legacy = self.root / ".workflow" / "flow" / "archive"
        legacy.mkdir(parents=True, exist_ok=True)
        legacy_dirty = legacy / "req-88-old" / "MARKER.txt"
        legacy_dirty.parent.mkdir(parents=True, exist_ok=True)
        legacy_dirty.write_text("do-not-touch", encoding="utf-8")

        folder_name = "req-55-primary-lands"
        _seed_requirement_dir(self.root, folder_name)
        _seed_requirement_state(
            self.root, "req-55", "primary-lands", "Primary Lands"
        )
        _write_runtime(
            self.root,
        ...
        # 关键断言 1：新归档落到 primary（artifacts/main/archive/requirements/）
        primary_target = (
            self.root
            / "artifacts"
            / "main"
        ...
        self.assertTrue(
            ...
            f"新归档应落到 primary {primary_target}；"
            f"legacy 非空不得绑架新归档路径。",
        )

        # 关键断言 2：legacy 历史数据保持不动（由 chg-02 的 migrate 负责搬）
        self.assertTrue(
            legacy_dirty.exists(),
            "legacy 历史数据应保持不动",
        )
        self.assertEqual(
            legacy_dirty.read_text(encoding="utf-8"),
            "do-not-touch",
        )

        # 关键断言 3：新 req 的目录不得出现在 legacy 路径下
        wrong_legacy = legacy / folder_name
        wrong_legacy_sub = legacy / "requirements" / folder_name
        self.assertFalse(
            wrong_legacy.exists(),
            f"新归档不得落到 legacy {wrong_legacy}",
        )
        self.assertFalse(
            wrong_legacy_sub.exists(),
            f"新归档不得落到 legacy {wrong_legacy_sub}",
        )
