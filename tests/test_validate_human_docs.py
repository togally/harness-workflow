"""对人文档落盘校验测试（req-39（对人文档家族契约化 + artifacts 扁平化）/ chg-02（validate_human_docs 重写 + 精简废止项））。

覆盖 ``src.harness_workflow.validate_human_docs.validate_human_docs`` 的核心行为：

- req-39+（新规扁平路径）：req 级文档齐全 + chg-NN- 前缀文件命中时 items 全 ok；
...
class ValidateHumanDocsTest(unittest.TestCase):
    ...
    def test_all_docs_present_returns_green(self) -> None:
        """新规 req-39：扁平路径全部文档齐全时全 ok。"""
        chg_ids = ("chg-01",)
        req_dir = self._make_req_flat(req_id="req-99", slug="demo", chg_ids=chg_ids)
    ...
    def test_missing_docs_reported(self) -> None:
        """新规 req-99：缺 需求摘要.md 时报 missing。"""
        chg_ids = ("chg-01",)
        req_dir = self._make_req_flat(req_id="req-99", slug="demo", chg_ids=chg_ids)
        self._write_all_req_flat_docs(req_dir, chg_ids=chg_ids)
        # 故意删掉 需求摘要.md
    ...
    def test_validate_removed_testing_acceptance_docs(self) -> None:
        """AC-6（精简）：format_report 输出中不含 测试结论 / 验收摘要 字串。"""
        chg_ids = ("chg-01",)
        req_dir = self._make_req_flat(req_id="req-99", slug="demo", chg_ids=chg_ids)
        self._write_all_req_flat_docs(req_dir, chg_ids=chg_ids)
        kind, target_id, items = validate_human_docs(self.root, "req-99")
