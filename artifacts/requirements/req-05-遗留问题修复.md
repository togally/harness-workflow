...
- **chg-01** chg-01-resolve前缀匹配：为 `resolve_change_reference` 添加前缀匹配逻辑，使其与 `resolve_requirement_reference` 行为一致。修复后 `harness change --requirement req-xx`（短 ID）和 `harness archive req-xx`（短 ID）均能正确解析含标题的目录名。
- **chg-02** chg-02-testing-acceptance阶段：在 `WORKFLOW_SEQUENCE` 中的 `executing` 和 `done` 之间插入 `testing` 和 `acceptance` 两个结构化自验证阶段，使 `harness next` 按 `executing -> testing -> acceptance -> done` 推进。
- **chg-03** chg-03-角色文件完善：确认并完善 `context/roles/testing.md` 和 `context/roles/acceptance.md` 的内容，使其包含明确的结构化自验证 checklist：
- **chg-04** chg-04-文档更新：1. 开发文档增加「本地开发验证」步骤，说明源码变更后需执行 `pipx inject harness-workflow . --force`
